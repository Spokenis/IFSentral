...
try {
    ...
    // Validar que todos os dispositivos dos datasets pertencem a este projeto
    // (evita que um usuário aponte um gráfico para device_id de outro projeto)
    $devicesStmt = $conn->prepare("SELECT id FROM devices WHERE project_id = ? AND deletedAt IS NULL");
    $devicesStmt->execute([$data->project_id]);
